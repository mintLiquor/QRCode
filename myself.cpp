...
void QRcode::display()
{
	...
	cout << "纠错码级别:" << version_level << endl;
	cout << "最大容量:" << num_code_word_without << endl;
	cout << "编码数据:";

	for (int i = 0; i < num_code_word_without; i++)
	{
		cout << int(code_word_final[i]) << ',';
	}
	cout << endl;
	cout << "格式信息" << format_data << endl;
}